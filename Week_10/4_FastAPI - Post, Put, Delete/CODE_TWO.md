Absolutely. Since you want to move from the **basic CRUD** to **advanced CRUD**, let's keep the same `Patient` model and JSON database, but add:
...
## Now you have these APIs

| Operation      | Method   | URL              |
| -------------- | -------- | ---------------- |
| Create         | `POST`   | `/patients`      |
| Read all       | `GET`    | `/patients`      |
| Read one       | `GET`    | `/patients/1`    |
| Full update    | `PUT`    | `/patients/1`    |
| Partial update | `PATCH`  | `/patients/1`    |
| Delete         | `DELETE` | `/patients/1`    |
| Search         | `GET`    | `/search`        |
| Pagination     | `GET`    | `/patients-page` |
...
`PUT` means **replace/update the complete patient**.
...
`PATCH` means **update only the fields you want**.
...
```text
GET /search?city=Hyderabad
```

or:

```text
GET /search?gender=male&min_age=20&max_age=40
```

### Pagination

```text
GET /patients-page?page=1&limit=2
```

Second page:

```text
GET /patients-page?page=2&limit=2
```

### Important concept

The progression you should learn is:
...
For your current preparation, **JSON-based advanced CRUD → SQLAlchemy CRUD** is a good next step.
